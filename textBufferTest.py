...
class class_bufferPosn(object):
	...
	def update(self,increment,count):
		if increment and (self.usedSize == self.bufferMaxSize):
...
class class_text_buffer(object):
	...
	def __init__(self,config,logtype,logTime):
		...
		print(" Buffer Init for : ",self.config.progName," with a size of : ",self.config.textBufferLength, \
			 " and  width of : ", len(self.config.headings) + 1, " including time stamp")
		if not os.path.exists('log'):
		    os.makedirs('log')

		self.sourceRef = 0 # a number used to control prevention repeat messages
		...
		print("self.html_FileName : ",self.htmlFileName)
		print("self.html_FileName_save_as : ",self.htmlFileNameSaveAs)
		print("self.wwwFileName : ",self.wwwFileName)
	...
	# Set up all the parts needed for the HTML file
...
<tbody>"""
	...
	def updateBuffer(self,appnd,count):
		i = 0
		for heading in self.config.headings:
			self.dta[self.bufferPosn.mostRecentPosn][i] = str(self.lineValues[heading])
			i += 1
		self.bufferPosn.update(appnd,count)

		if self.config.logBufferFlag and appnd:
			...
			if fileExists(self.wwwFileName):
				try:
					...
					print("Failed to copy log file to www because this not there: ",self.wwwFileName)
	...
	def pr(self,appnd,count,logTime):
...
# Headings are:    Time,Count,Hour In Day,Position,Heading5,Heading6,Reason,Message
		...
		logBuffer.pr(increment,config.scanCount,logTime)
		increment = False
		reason = ""
		message = ""
